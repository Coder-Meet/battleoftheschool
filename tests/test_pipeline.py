...
def test_shared_physical_settings_and_two_mm_origin_policy(monkeypatch):
    ...
    assert len(seen) == 2
    for current in seen:
        assert current.native_contrast_scale == .9
...
def test_strict_escape_hatch_and_explicit_custom_threshold(monkeypatch, model_path):
    ...
    assert calls[0].native_contrast_scale == 1.2
...
def test_recovery_requires_strict_and_preserves_root_guard(monkeypatch):
    monkeypatch.setattr(pipeline, 'detect', lambda *args: pytest.fail('unexpected detector'))
    with pytest.raises(ValueError, match='requires --pipeline strict'):
        pipeline.run_pipeline(None, None, recover_connected_origins=True)
    seen = []
    def recover(image, mask, config, *, require_root):
        seen.append((config.native_contrast_scale, require_root))
        return detection([], config)
    monkeypatch.setattr(pipeline, 'detect_connected', recover)
    _, metadata = pipeline.run_pipeline(None, None, workflow='strict', recover_connected_origins=True)
    assert seen == [(1.2, True)]
    assert metadata['recover_connected_origins'] is True
