...
The earlier strict release evidence and its replay scripts remain in git history; four historical receipts are kept under `presentation/evidence/`. The current deck and graphics use [fusion package verification](presentation/evidence/judge-fusion-verification.json) and the current validation receipt. Fusion's 25-case offline replay averages **11.60 s**, peaks at **52.67 s** per case and **1499 MiB sampled RSS** on four-core Linux. Native organizer-Windows timing remains unmeasured.

The 24-case synthetic comparison gives fusion **47 TP / 11 FP / 2 FN, F1 0.8785**, versus strict **46/0/3, F1 0.9684**. Fusion adds seven FPs in `touching_vein_and_calcification` and four in `negative_controls_only`, both seed 31415. Thus the old zero-negative-control claim does not apply to fusion. This known FP regression is retained with the requested development operating point and is the next filter-hardening target. Full results are recorded in the receipt.
