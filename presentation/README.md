...
Manrope body type. Generated slides and films stay outside Git; the
[public README gallery](../docs/media/README.md) is committed for reuse.

Presenters: read the [complete briefing and judge Q&A](../PRESENTER_BRIEFING.md).
For the public entry, use the [Devpost copy and checklist](../DEVPOST_SUBMISSION.md).

## Start with the delivered kit

Published downloads: [current fusion submission kit](https://github.com/Coder-Meet/battleoftheschool/releases/download/branchseed-submission-fusion-2026-09-13/branchseed-submission-fusion-kit.zip)
...
Extract the entire ZIP before opening `START_HERE.html`, then `presentation/index.html`. Keep `assets/`
...
The committed README images are generated by `build-project-media.py` from
the preserved recording and current fusion receipts. See the
[media instructions](../docs/media/README.md).

To generate offline HTML plus editable Word documents for the submission and
presenter briefing:

```bash
uv pip install --python .venv313/bin/python -r presentation/requirements-docs.txt
.venv313/bin/python presentation/build-project-docs.py --pdf \
  --deck outputs/fusion-presentation-kit \
  --slides outputs/fusion-presentation-kit/branchseed-slides.pdf \
  --showcase outputs/product-showcase/branchseed-showcase.mp4
```

`--pdf` additionally requires LibreOffice on the authoring machine. Without
it, omit the flag; HTML and Word generation still work. Output is
`outputs/fusion-submission-pack`; open `START_HERE.html` for navigation.
Build the deck below first. `--deck` includes its assets, editable PPTX and
script, with Word/PDF/HTML script copies. `--showcase` and `--slides` add the
separate film and slide PDF. These authoring dependencies do not affect inference.
...
# Generate fusion detector results in a fresh output directory.
OPENBLAS_NUM_THREADS=4 OMP_NUM_THREADS=4 \
  .venv313/bin/python batch.py --data-root data --output-dir outputs/fusion-presentation-predictions

# Use a real Explorer screenshot from the delivered kit as slide 5's cue card.
.venv313/bin/python presentation/build.py \
  --predictions outputs/fusion-presentation-predictions \
  --poster /path/to/explorer-screenshot.png
node presentation/build-pptx.cjs outputs/fusion-presentation-kit
```

Outputs default to `outputs/fusion-presentation-kit`. To change copy, timing or composition,
...
The deck reflects **score-before-merge fusion**: 1 mm working grid, native
contrast scale 0.9, bundled logistic threshold 0.15 and strict-first merging
within 3 mm.

- Five reused cases, 19 judge-approved AI-assisted/non-exhaustive targets:
  14 TP / 4 FP / 5 FN; precision 0.7778, recall 0.7368, F1 0.75676,
  daughter-count MAE 1.8. This is development-reference agreement.
- Topology, 24 synthetic cases: 47 TP / 11 FP / 2 FN; F1 0.8785.
  Includes four negative-control detections; separate from real-reference results.
- Fusion ran all 25 scans with four-core Linux affinity: 11.60 seconds mean,
  52.67 seconds maximum and 1499 MiB sampled peak RSS. Organizer-Windows timing is pending.
- The three CT visuals use fusion outputs for subjects001–003.
  Their overlays are visual checks, not complete annotations.

Sources: [fusion package verification](evidence/judge-fusion-verification.json)
and [fusion reference/topology validation](../docs/fusion-restored-validation.json).
Older strict receipts remain historical evidence and are not inputs to current graphics.
...
combines classical image processing with a bundled logistic classifier.
A classifier cannot recover a branch never proposed.
...
6. Use the fusion default, preload subject018 and keep subject001 as backup.
