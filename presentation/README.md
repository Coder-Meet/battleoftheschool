...
the preserved recording and frozen strict receipts. See the
...
`outputs/project-submission-pack`; open `START_HERE.html` for navigation.
Optionally pass `--showcase /path/to/branchseed-showcase.mp4` and
`--slides /path/to/branchseed-slides.pdf` to include copies of the separate
film and live-demo PDF. These authoring dependencies do not affect inference
or the Explorer.
